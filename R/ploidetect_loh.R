ploidetect_loh <- function(purity, CNA_object){
  ...
  segments <- CNA_object_mafs %>% group_by(chr, segment) %>% summarise(median_maf = median(median_maf), median_segment = median(median_segment), CN = median(CN))
  ...
  for(segment in 1:nrow(segments)){
    ...
    if(CN %in% names(maf_possibilities)){
      ...
      decision <- which.min(abs(segments$median_maf[segment] - candidates))
