...
The density plots are a visual representation of Ploidetect's attempts at determining the tumour purity. They can be thought of as a smoothed histogram of the read depth. Generally we observe that the density plots show a few peaks, where each peak represents a cluster of genomic regions with a similar coverage. We assume that a bunch of regions with similar coverage means that those regions have the same copy number. Ploidetect tries to select a set of peaks that are regularly spaced, since the read depth difference between a 1-copy region and a 2-copy region should be the same as the difference between a 2-copy region and a 3-copy region. The first density plot shows where all peaks have been called in the data. Each subsequent density plot is the result of Ploidetect selecting a set from those peaks to estimate the difference in coverage between integer copy numbers in the tumour genome. The plot for a given model, combined with the results, is communicating to you the copy number and allele frequency of the different copy number regions of the genome. Ploidetect does its best to use that information to estimate tumour purity, but by providing them to you, you are free to second-guess the results and provide a more optimal model if necessary.
...
These are the models that Ploidetect used to predict tumour purity in the sample. Given that this was a 100% pure sample, it did pretty well with 95.5%. But how do we make sure the results are fine?
...
When you manually review Ploidetect's models, what you are looking for is fitting distinct, regularly spaced peaks. Here's an example where Ploidetect is close, but there is a better model which predicts a more reasonable tumour purity. 

```{r}
dat <- read.table(Sys.glob("../../coverage/POG003/binned/1/*processed.bed")[1], stringsAsFactors = F, header = T)
str(dat)
devtools::load_all(pkg = "../")
result_2 <- ploidetect(dat, normal = 2, tumour = 1, avg_allele_freq = 4, window_id = 3, window_size = 5, GC = 6, verbose = F, bw = 2000, CNA_call = F)
```

```{r}
(tc_2 <- result_2$TC_calls)

plots <- result_2$plots
