linearTransform <- function(x, tumour, normal, avg_allele_freq, window_id, slope = 0, verbose = verbose){
  ...
  fitted_model <- cbind.data.frame(x$y_raw, x$x_raw, xfit$residuals, x$normalized_size, x$GC, x$size) %>% `row.names<-`(row.names(x))
  ...
  filtered <- left_join(x = filtered, y = data.frame("maf" = x$maf, "wind" = x$window, stringsAsFactors = F), by = "wind", stringsAsFactors = F)
